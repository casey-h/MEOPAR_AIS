...
usage_message = """
Usage: seg_interp_into_grids.py inputtrackfilename interp_field1 interp_field2 
inputgridfilename outputinttextfilename output_interp_field ... Reads the 
specified segment and grid (shapefile) filenames, and calculates the 
interpolated field along the tracks into the grids. Assumes input layer 
contains sog simple (two point) segments with sog values stored for the first 
and last points. Generates a text file containing the segment data plus the 
interpolated sog and grid id under outputinttextfilename. Developed as part of 
...
in_grid_file = sys.argv[4]
out_int_filename = sys.argv[5]
output_interp_field = sys.argv[6]
...
out_intersect_records.write(output_interp_field + ",grd_len_km,grid_id\n")
...
for grid_fid in range(0, grid_feature_count):
    ...
    for line_fid in list(index.intersection((xmin, xmax, ymin, ymax))):
        ...
        if grid_geometry.Contains(line_geometry):
            ...
            # Output the mean value to the output layer.
        ...
        # length over the cell.
            ...
            # Output the interpolated value to the output layer.
            out_intersect_records.write(str(interp_calc) + ",")
