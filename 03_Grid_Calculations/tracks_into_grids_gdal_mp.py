...
#VVVV Define this as MP task? Assign over for loop above
## Pass just FIDs and wkts -- both pickle
##      Generate list of FIDS an wkts:
##      [FID, wkt] = map(get_fid_and_wkt, list(index.intersection((xmin, xmax, ymin, ymax))), line_layer)
##      - Can a non-list argument be passed to map
## Output is either nothing or FIDs of line, grid, wkt of intersection result
## Outputs passed via ?Queue?
## Call either as apply_async or map (map_async might also be appropriate)
        ...
        if grid_geometry.Intersects(line_geometry):
            ...
            # DEBUG -- Determine the type of geometry in the intersection.
            # In testing, this clause was never visited, as only simple 
            # (i.e. two-point) lines were utilized as input.
            """
