...
def split_pre_tracks(in_filename, split_message_file_directory, split_file_directory, split_other_file_directory, out_filename_prefix):
    ...
    with open(in_filename,'r') as in_vessel_records:
        ...
        for line in in_records_bar(in_vessel_records):
            ...
            #Lines from TAIS data are completely pipe, not tab+pipe delimited,
            #and the schema depends on the type. Format always leads with ISO time, then message type.
            #
            ...
            pipetokenizedline = line.split('|');
            input_date = pipetokenizedline[0]
            input_msg_type = pipetokenizedline[1]

            # Output tokenized fields according to the message type observed.
            #1_2_3
            # example type 1 message: 20140604T000000.078Z|1|0|316012751|n/a|0.0|0.1|0|-123.123985|49.29111|165.1|226|58|0|0|49232
            # Input schema: ext_time, message_id, repeat_ind, mmsi, nav_stat, ROT, SOG, pos_acc, lon, lat, cog, tr_hdg, timestamp, maneuver, RAIM, radio_stat
            if(input_msg_type in ("1", "2", "3")):

                # ext_timestamp(0),msgid(1),mmsi(3),nav_stat(4),sog(6),cog(10),tr_hdg(11),lat(9),lon(8),pos_acc(7)
                out_message_records[0].write((pipetokenizedline[0].strip() + "," +
                pipetokenizedline[1].strip() + "," +
                pipetokenizedline[3].strip() + "," +
                pipetokenizedline[4].strip() + "," +
                pipetokenizedline[6].strip() + "," +
                pipetokenizedline[10].strip() + "," +
                pipetokenizedline[11].strip() + "," +
                pipetokenizedline[9].strip() + "," +
                pipetokenizedline[8].strip() + "," +
                pipetokenizedline[7].strip() + "\n")
                )

            #18
            # example type 18: 20140604T000026.280Z|18|0|316017734|0|9.7|0|-123.146226667|49.2813133333|120.3|n/a|25|0|1|0|1|1|1|0|1|917510
            # Input schema: ext_time, message_id, repeat_ind, mmsi, reserved, SOG, pos_acc, lon, lat, COG, tr_hdg, timestamp, regional, cs_unit, display, dsc, band, msg22, assigned, RAIM, radio_stat
            elif(input_msg_type in ("18")):

                #ext_timestamp(0),msgid(1),mmsi(3),nav_stat -- , sog(5), cog(9), tr_hdg(10), lat(8), lon(7), pos_acc(6)
                out_message_records[1].write((pipetokenizedline[0].strip() + "," +
                pipetokenizedline[1].strip() + "," +
                pipetokenizedline[3].strip() + "," +
                "" + "," +
                pipetokenizedline[5].strip() + "," +
                pipetokenizedline[9].strip() + "," +
                pipetokenizedline[10].strip() + "," +
                pipetokenizedline[8].strip() + "," +
                pipetokenizedline[7].strip() + "," +
            ...
            #27
            # need example for type type 27 - none in current example (DIGITALYACHTAISNET1302-0097-01_20140604T000000.000Z.txt)
            # Input schema: ext_time, message_id, repeat_ind, mmsi, pos_acc, RAIM, nav_status, lon, lat, SOG, COG, GNSS, spare
            elif(input_msg_type in ("27")):
                
                #ext_timestamp(0),msgid(1),mmsi(3),nav_stat(6),sog(9),cog(10),tr_hdg --,lat(8),lon(7),pos_acc(4)

                out_message_records[2].write(pipetokenizedline[0].strip() + "," + 
                pipetokenizedline[1].strip() + "," + 
                pipetokenizedline[3].strip() + "," + 
                pipetokenizedline[6].strip() + "," + 
                pipetokenizedline[9].strip() + "," + 
...
def generate_short_GIS(segment_split_directory, gis_directory, out_filename_prefix, max_elapsed_time, outputEPSG):
    ...
    for segment_split_filename in in_segment_split_files_bar(glob(segment_split_directory  + sep + "*.txt")):
        ...
        with open(segment_split_filename,'r') as in_track_records:
            ...
            for line in in_track_records:
                ...
                # before initializing the next. Swapped <> -> != for python 3 20180107
                    ...
                    if(track_len > 1):
                        ...
                        # time is less than the threshold. Swapped <> -> != for python 3 20180107
    ...
    # Print out the number of preserved and discarded segments. Adjust to print function / python3 CH 20180107 (Add parens)
    ...
    # Print out the thresholds applied in processing. Adjust to print function / python3 CH 20180107 (Add parens)
...
### (End) This condition suggests an orphaned point (the existing -1/"prev_" point)
                        ...
                        # any remaining track points. Swapped <> -> != for python 3 20180107
...
def generate_threshold_GIS(segment_split_directory, gis_directory, out_filename_prefix, outputEPSG):
    ...
    # Print a message about the stage of processing. Adjust to print function / python3 CH 20180107 (Add parens)
...
inputfilename - The location/name of the input file of formatted AIS data (as parsed by DMAS_TAIS_NM4_parsing.py).
