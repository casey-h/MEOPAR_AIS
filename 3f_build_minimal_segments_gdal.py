...
# build_minimal_segments_gdal.py
...
#Minimum inferred speed boundary (knots)
min_speed_bound_kts = 1

#Maximum inferred speed boundary (knots)
max_speed_bound_kts = 86.3930885411603

#Max elapsed time between points for segment to be created (seconds)
# Satellite overflight for eE circa 2015 cited as <= 90 minutes 5400 
# -- 7200 considered a generous buffer (2016-04-01).
max_elapsed_time = 7200

# If the wrong number of arguments is provided, display an usage message.
if (len(sys.argv) < 4):
    print 'Usage: build_minimal_segments_gdal.py outputEPSG outputshapefileprefix inputfilename [inputfilename ...] ... \n Reads the specified input filenames, presumed to correspond to csv files of waypoint data, one per mmsi, ordered by timestamp. (output of divide_tracks.py). Generates shapefiles of corresponding trajectories. Output EPSG is the target for the generated files (e.g. BC Albers - 3005; UTM 20N NAD83 - 2961). Input is presumed to be WGS84 (AIS default). Developed in support of the NEMES project (http://www.nemesproject.com/).\n'
    quit()
...
out_line_filename = os.path.basename(sys.argv[2] + "_lines")
out_line_directory = os.path.dirname(sys.argv[2] + "_lines")
                ...
                # before initializing the next.
                    ...
                    if(track_len > 1):
                        ...
                        if (inMaxSeconds <> inMinSeconds) and (inMaxSeconds - inMinSeconds < max_elapsed_time):
                            ...
                            # Calculate the length of the segment under BC Albers Projection and 
                            # speed in knots  (https://en.wikipedia.org/wiki/Knot_(unit))
                            track_len_m = track_obj.Length()
                            speed_knots = (track_len_m / 1852) / ((inMaxSeconds - inMinSeconds) / float(3600))
                            ...
                            if ((speed_knots > min_speed_bound_kts) and (speed_knots < max_speed_bound_kts)):
                                ...
                                speed_discarded_count = speed_discarded_count + 1
...
print "Completed, " + str(preserved_count) + " segments generated, " + str(speed_discarded_count) + " discarded for invalid speed, " + str(time_discarded_count) + " discarded for invalid time.\n"
